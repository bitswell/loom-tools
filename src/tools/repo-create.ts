...
export const repoCreateTool: Tool<RepoCreateIn, RepoCreateOut> = {
  ...
  handler: async (input) => {
    ...
    // 1. Check if repo already exists (idempotent)
    const viewResult = await exec('gh', ['repo', 'view', fullName, '--json', 'name'], tmpdir());
    ...
    try {
      ...
      // Write README (no shell injection — use writeFileSync)
      writeFileSync(join(cloneDir, 'README.md'), `# ${input.name}\n`);
      ...
      // Write ruleset JSON to temp file to avoid shell injection
      const rulesetFile = join(tmpdir(), `loom-ruleset-${Date.now()}.json`);
      try {
        writeFileSync(rulesetFile, rulesetBody);
        const protectResult = await exec(
          'gh',
          ['api', `repos/${fullName}/rulesets`, '-X', 'POST', '--input', rulesetFile],
          cloneDir,
        );
        ...
        try { unlinkSync(rulesetFile); } catch { /* ignore */ }
      }
